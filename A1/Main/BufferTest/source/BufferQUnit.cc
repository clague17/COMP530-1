...
#ifndef BUFFER_QUNIT_H
#define BUFFER_QUNIT_H
...
//
	...
	{
		cout << "create manager..." << flush;
		MyDB_BufferManager myMgr(64, 50, "tempDSFSD");//100
		cout << "get page..." << flush;
		MyDB_TablePtr table1 = make_shared <MyDB_Table>("table1", "file1");
		vector<MyDB_PageHandle> pages(51);//101
		for (int i = 0; i < 51; i++) {
			pages[i] = myMgr.getPage(table1, i);
		}
		cout << "get bytes..." << flush;
	...
	{
		cout << "create manager..." << flush;
		MyDB_BufferManager myMgr(64, 16, "tempDSFSD4_");
		cout << "get page..." << flush;
		...
		for (int i = 0; i < 50; i++) {
			...
			for (int j = 0; j < 64; j++) {
                if (bytes[i][j] != c) {
                    flag8 = false;
                    cout << "Error: " << "i = " << i << ", j = "<< j << ", wrong : "<< bytes[i][j] << ", right = " << c <<endl;
                }
			}
		}
